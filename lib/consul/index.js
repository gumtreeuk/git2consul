...
var delete_content_from_consul = function(key_name, recurse, cb) {
  ...
  consul.kv.del({'key': key_name, token: token, recurse: recurse}, function(err) {
    /* istanbul ignore if */
...
var file_deleted = function(branch, file, cb) {
  ...
  delete_content_from_consul(key_name, branch.expand_keys, cb)
};
...
exports.deleteBranch = function(branch, cb) {
  file_deleted(branch, "", function(err) {
    if (err) return cb(err);
    exports.ref_deleted(branch, function(err) {
      if (err) return cb(err);
    });
  });
};

exports.ref_deleted = function(branch, cb) {
  // Prepend branch name to the KV so that the subtree is properly namespaced in Consuls
  var key_name = create_key_name(branch, branch.name + '.ref', true);

  delete_content_from_consul(key_name, false, cb)
};
/**
